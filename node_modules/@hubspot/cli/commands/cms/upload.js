...
async function handler(args) {
    ...
    if (stats.isFile()) {
        ...
        uiLogger.log(commands.cms.subcommands.upload.uploading(src, dest, derivedAccountId));
        ...
        if (args.clean) {
            ...
            if (!args.force) {
                cleanUpload = await confirmPrompt(commands.cms.subcommands.upload.confirmCleanUpload(dest, derivedAccountId), { defaultAnswer: false });
            }
            if (cleanUpload) {
                try {
                    await deleteFile(derivedAccountId, dest);
                    uiLogger.log(commands.cms.subcommands.upload.cleaning(dest, derivedAccountId));
                }
                catch (error) {
                    uiLogger.error(commands.cms.subcommands.upload.errors.deleteFailed(dest, derivedAccountId));
                }
            }
        }
...
const builder = makeYargsBuilder(uploadBuilder, command, describe, {
