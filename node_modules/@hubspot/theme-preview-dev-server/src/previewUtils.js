...
exports.buildDevFileHashTable = buildDevFileHashTable;
...
const getPortalDomains = async (accountId) => {
    try {
        const { data } = await (0, domains_js_1.fetchDomains)(accountId);
        return data.results;
    }
    catch (error) {
        return [];
...
const trackPreviewEvent = async (action, options) => {
    let allowUsageTracking = false;
    try {
        const config = (0, config_1.getConfig)();
        allowUsageTracking = config.allowUsageTracking || false;
    }
    catch (err) { }
    if (!allowUsageTracking) {
        return;
    }
    const defaultAccount = (0, config_1.getConfigDefaultAccountIfExists)();
...
const getAuthType = (accountId) => {
    ...
    if (accountId) {
        const accountConfig = (0, config_1.getConfigAccountIfExists)(accountId);
...
async function buildDevFileHashTable(src, dest, filePaths) {
    return filePaths.reduce((acc, filePath) => {
        const relativePath = path_1.default.relative(src, filePath);
        const destPath = path_1.default.join(dest, relativePath);
        // Files stored in BE have a preceding /
        acc[`/${destPath}`] = getMd5Hash(filePath);
        return acc;
    }, {});
}
// Get md5 hash of file given its path
