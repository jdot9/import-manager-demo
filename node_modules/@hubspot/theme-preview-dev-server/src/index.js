...
const initialPreviewBufferUpload = async (sessionInfo, filePaths, uploadOptions) => {
    ...
    (0, previewUtils_js_1.trackPreviewEvent)('server-init', {
...
const preview = async (accountId, src, dest, { notify, filePaths, skipUpload, noSsl, port, startProgressBar, handleUserInput, resetSession }) => {
    (0, logger_1.setLogLevel)(logger_1.LOG_LEVEL.LOG);
    let accountConfig;
    try {
        accountConfig = (0, config_1.getConfigAccountById)(accountId);
    }
    catch (err) {
        logger_1.logger.error(`Failed to get account config for ${accountId}: ${err}`);
        return;
    }
    ...
    let tokenStatus;
    try {
        const { data } = await (0, session_1.initiateSession)(accountId, resetSession);
    ...
    const absDest = `@preview/${sessionToken}/${dest}`;
    const sessionInfo = {
        src,
        dest: absDest,
        fakeDest: dest,
        ...
        personalAccessKey: accountConfig.authType === auth_1.PERSONAL_ACCESS_KEY_AUTH_METHOD.value ? accountConfig.personalAccessKey : undefined,
    ...
    if (!skipUpload) {
        if (tokenStatus === 'NEW') {
            console.log('Uploading for new session... \n');
            const uploadOptions = startProgressBar(filePaths.length);
            ...
            const hashes = await (0, previewUtils_js_1.buildDevFileHashTable)(src, absDest, filePaths);
            let res;
            try {
                res = (await (0, session_1.diffDeveloperFileHashes)(accountId, sessionToken, hashes));
            }
            catch (err) {
                logger_1.logger.error(`Failed to diff developer file hashes: ${err.response.data.message}`);
                logger_1.logger.error(`Please try again, or use --resetSession if errors continue.`);
                return;
            }
            const { changedFiles, addedFiles, deletedFiles } = res.data;
            const filesToUpload = [...changedFiles, ...addedFiles].map(filePath => {
                // Strip the preceding / that BE adds
                const relPath = path_1.default.relative(absDest, filePath.substring(1));
                return path_1.default.join(src, relPath);
            });
            if (filesToUpload.length) {
                const uploadOptions = startProgressBar(filesToUpload.length);
                await initialPreviewBufferUpload(sessionInfo, filesToUpload, uploadOptions);
            }
        }
    }
